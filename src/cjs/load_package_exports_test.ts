...
describe("loadPackageExports", () => {
  it("should return undefined if the package.json does not exist", async () => {
    await expect(loadPackageExports(virtualPackageDir, ".", { conditions: [] }))
      .resolves
      .toBe(undefined);
  });
  ...
  it("should return undefined if the package.json does not contain `exports` field", async () => {
    await expect(
      loadPackageExports(noPjson.packageURL, ".", { conditions: [] }),
    )
      .resolves
      .toBe(undefined);
  });
  ...
  it("should return result", async () => {
    await expect(
      loadPackageExports(pjsonExportsSugar.packageURL, ".", { conditions: [] }),
    )
      .resolves
      .toEqual({
